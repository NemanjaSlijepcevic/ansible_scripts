...
- `-p 5432:5432` publishes Postgres on the host as well, for connections from outside the `proxy` network (e.g. pgAdmin, other hosts' promtail/telegraf, or manual `psql` from the host itself).
