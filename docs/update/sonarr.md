...
Sonarr is the TV series manager. You add a series once; from then on Sonarr tracks which episodes
exist, which are missing, and which have aired, searches the indexers for them, hands the torrent to
the download client, and when the download finishes imports the file into the library — renaming it
into the naming scheme, moving it onto the TV drive, and telling the media server to rescan.

It runs as a single container on the storage host, on the shared `proxy` bridge network with a fixed
address, and is published by the reverse proxy at `https://sonarr.your-domain.com`. Its database is
**not** local: it stores everything in the central PostgreSQL server, in two databases (`sonarr` and
`sonarr-log`), over a mutually-authenticated TLS connection.
...
- **`prowlarr`** on port 9696 — the indexer manager. Sonarr does not configure indexers itself; the
  indexer manager pushes them in, which is why you will not add any by hand here.
- **`jellyfin`** on port 8096 — the media server, notified after every successful import so a
  finished episode appears in the library within seconds instead of at the next scheduled scan.
...
```bash
curl -s -o /dev/null -w '%{http_code}\n' https://sonarr.your-domain.com/ping     # expect 200
curl -s -o /dev/null -w '%{http_code}\n' https://sonarr.your-domain.com/         # expect 302 to the portal
```

**The service's DNS name resolves to this host**

```bash
dig +short sonarr.your-domain.com
```
...
```bash
ls -l ./data/certs/
sudo openssl x509 -in ./data/certs/sonarr.crt -noout -subject -dates
```

You need three files on this host: `./data/certs/sonarr.crt`, `./data/certs/sonarr.key`, and the
...
necessarily the file name. The file is called `sonarr.crt` for convenience; the role inside it is the
...
```bash
docker exec -it postgres-db psql -U postgres -c '\l' | grep -E 'sonarr'
docker exec -it postgres-db psql -U postgres -c '\du' | grep <db-user>
```

Both `sonarr` and `sonarr-log` must exist and be owned by the login role, otherwise the first start
...
CREATE DATABASE "sonarr"     OWNER "<db-user>";
CREATE DATABASE "sonarr-log" OWNER "<db-user>";
EOF
```
...
You also need its API key for Step 7. Read it from the media server itself:

```bash
curl -s https://jellyfin.your-domain.com/Auth/Keys -H 'X-Emby-Token: <jellyfin-api-key>' \
  | jq -r '.Items[] | "\(.AppName)\t\(.AccessToken)"'
```

**The TV and download drives are mounted, and you know the UID that owns them**

```bash
findmnt -no TARGET,SOURCE,FSTYPE <tv-path> <downloads-path>
ls -ld <tv-path>/Serije <downloads-path>/Download
stat -c '%u %g %n' <tv-path> <downloads-path>
```
...
sudo chmod 0755 <tv-path> <downloads-path>
sudo mkdir -p <downloads-path>/Download/complete/sonarr
sudo chown <puid>:<pgid> <downloads-path>/Download/complete/sonarr
sudo chmod 0755 <downloads-path>/Download/complete/sonarr
```
...
section. The category folder is where the download client will place finished TV torrents, and its
name must exactly equal the category label you configure in Step 5 (`sonarr`). Creating it up front
...
cd <deploy-dir>
mkdir -p ./data/sonarr
sudo chown <username>:<pgid> ./data/sonarr
sudo chmod 0755 ./data/sonarr
```

**Explanation**: only the parent is created here. The container creates `./data/sonarr/config` itself
on first start, as its own user, and populates it with `config.xml`, the log files and the cached
series artwork. The parent belongs to your deploy account so you can inspect and back it up without
`sudo`.
...
docker run -d \
  ...
  -e PGID=<pgid> \
  -e TZ=<timezone> \
  -e SONARR__AUTH__APIKEY='<secret>' \
  -e SONARR__AUTH__METHOD=External \
  -e SONARR__AUTH__REQUIRED=DisabledForLocalAddresses \
  -e SONARR__SERVER__PORT=8989 \
  -e SONARR__POSTGRES__HOST=<postgres-host> \
  -e SONARR__POSTGRES__PORT=5432 \
  -e SONARR__POSTGRES__USER='<db-user>' \
  -e SONARR__POSTGRES__PASSWORD='<secret>' \
  -e SONARR__POSTGRES__MAINDB=sonarr \
  -e SONARR__POSTGRES__LOGDB=sonarr-log \
  -e PGSSLMODE=verify-ca \
  -e PGSSLCERT=/postgres-certs/sonarr.crt \
  -e PGSSLKEY=/postgres-certs/sonarr.key \
  ...
  --label 'traefik.http.routers.sonarr.middlewares=chain-auth@file' \
  --label 'traefik.http.services.sonarr.loadbalancer.server.port=8989' \
  --health-cmd 'curl --fail http://localhost:8989/ping || exit 1' \
  --health-interval 90s \
  --health-timeout 10s \
  --health-retries 3 \
...
application. They must match the owner of the TV and download directories, and must be the **same**
numbers the download client uses, because importing a finished episode is a rename or a hardlink from
...
`SONARR__AUTH__APIKEY` sets the API key from the outside instead of letting the application generate
...
`SONARR__AUTH__METHOD=External` means the application does not render its own login form: it trusts
...
`SONARR__AUTH__REQUIRED=DisabledForLocalAddresses`, requests arriving from private addresses — the
...
`SONARR__POSTGRES__MAINDB` and `__LOGDB` are two separate databases on purpose. The log database takes
...
library data, and you can truncate it wholesale without risking a single series record.
...
server's rules use certificate authentication, so the Common Name inside `sonarr.crt` is what actually
...
completed folder and the reported path does not exist here, producing the "downloaded episode could
not be imported" errors that are the single most common problem in this stack.
...
for i in $(seq 1 10); do
  curl -sf https://sonarr.your-domain.com/ping && { echo; echo "sonarr: ready"; break; }
  echo "waiting for sonarr..."; sleep 10
done
...
it too early. If the loop never succeeds, `docker logs sonarr` will name the database problem
directly.
...
BASE='https://sonarr.your-domain.com/api/v3'
...
`tvCategory` is the label attached to every torrent this service hands over. The download client turns
that label into the subdirectory it saves to — `Download/complete/sonarr`, the folder created in
Step 1 — and this service then looks there for finished downloads. The category string and the folder
name must match exactly; if you change one, change the other. `recentTvPriority: 0` puts recently
aired episodes in the normal queue rather than at the front of it.
...
service calls the media server's API and tells it to rescan the affected folder, so a finished
episode is playable within seconds instead of whenever the media server's own scheduled scan next
runs. Turning the scheduled scan down and relying on this hook is much cheaper on a spinning disk
than scanning the whole library every hour.
...
| --- | --- | --- |
| Indexer manager → this service | Pushes the indexer list in and keeps it in sync. You never add indexers here. | The indexer manager holds this service's API key and calls `http://sonarr:8989` |
| This service → download client | Sends the torrent with the category `sonarr` | `transmission:9091` on the bridge |
| Download client → disk | Writes the finished file into `Download/complete/sonarr` | Shared `<downloads-path>` mount |
| This service → library | Imports (hardlink or move) into `<tv-path>/Serije` and renames | Both are visible in the container as `/downloads` and `/tv` |
| This service → media server | "Rescan this folder" after every import | `jellyfin:8096` on the bridge |
| Subtitle manager → this service | Reads the series list and their file paths to know what needs subtitles | Calls `http://sonarr:8989` with this service's API key |
...
| Path | Contents |
| --- | --- |
| `./data/sonarr/config/` | `config.xml`, logs, cached series artwork. **Not** the database. |
| `./data/sonarr/config/logs/` | `sonarr.txt` and rotated copies — where import and indexer errors actually appear |
| `./data/certs/` | Client certificate, key and CA for the database connection; mounted read-only as `/postgres-certs` |
| `<tv-path>/Serije` | The TV library, mounted as `/tv` |
| `<downloads-path>/Download` | The whole download tree, mounted as `/downloads` |
| `<downloads-path>/Download/complete/sonarr` | Where the download client places finished TV torrents |
| PostgreSQL `sonarr` | Series, episodes, quality profiles, history — the real state |
| PostgreSQL `sonarr-log` | Application log rows; safe to truncate |
...
| `<puid>` / `<pgid>` | Numeric UID and GID the container runs as | Must equal the owner of the TV and download directories, and match the download client and media server | Steps 1, 3 |
...
| `<timezone>` | IANA timezone name | The host's, so the calendar and air dates read correctly | Step 3 |
| `<tv-path>` / `<downloads-path>` | Mount points of the TV and download drives | Where the drives are actually mounted; check with `findmnt` | Steps 1, 3 |
| `<secret>` (API key) | This service's API key | 32 hex characters: `openssl rand -hex 16`. Choose it before the first start | Steps 3, 5–8, and in the indexer and subtitle managers |
...
| `<db-user>` | Database login role | The shared media database account; must equal the Common Name in `sonarr.crt` | Step 3, Before you start |
...
curl -s https://sonarr.your-domain.com/ping
curl -s https://sonarr.your-domain.com/api/v3/system/status -H 'X-Api-Key: <secret>' \
  | jq '{version, isProduction, appData, startTime}'

# the database connection is the PostgreSQL one, not a local SQLite file
docker logs sonarr 2>&1 | grep -i -m5 'postgres\|database'
ls ./data/sonarr/config/*.db 2>/dev/null && echo "WARNING: a local database exists — the PostgreSQL settings were not applied"

# the download client, notifications and media server hook are registered
curl -s https://sonarr.your-domain.com/api/v3/downloadclient -H 'X-Api-Key: <secret>' | jq -r '.[] | "\(.name)\t\(.enable)"'
curl -s https://sonarr.your-domain.com/api/v3/notification   -H 'X-Api-Key: <secret>' | jq -r '.[] | .name'

# indexers arrived from the indexer manager
curl -s https://sonarr.your-domain.com/api/v3/indexer -H 'X-Api-Key: <secret>' | jq -r '.[] | "\(.name)\t\(.enableRss)"'

# nothing is unhealthy
curl -s https://sonarr.your-domain.com/api/v3/health -H 'X-Api-Key: <secret>' | jq -r '.[] | "\(.type)\t\(.message)"'

# the container can actually read and write both trees
docker exec sonarr ls -ld /tv /downloads /downloads/complete/sonarr
docker exec sonarr touch /tv/.write-test && docker exec sonarr rm /tv/.write-test && echo "library writable: ok"

# the download client is reachable from inside this container by name
docker exec sonarr curl -sf -o /dev/null -w 'transmission: %{http_code}\n' http://transmission:9091/transmission/web/
docker exec sonarr curl -sf -o /dev/null -w 'jellyfin: %{http_code}\n' http://jellyfin:8096/health
```
...
instead — stop, fix the variables, delete the file and start again before you add any series.
...
```bash
docker logs -f --tail 100 sonarr
tail -f ./data/sonarr/config/logs/sonarr.txt
```
...
curl -s https://sonarr.your-domain.com/api/v3/health -H 'X-Api-Key: <secret>' | jq

# items stuck in the queue
curl -s 'https://sonarr.your-domain.com/api/v3/queue?pageSize=100' -H 'X-Api-Key: <secret>' \
...
docker exec -it postgres-db psql -U postgres -d sonarr-log -c 'TRUNCATE "Logs";'
```
...
docker rm -f sonarr
sudo rm -rf ./data/sonarr
docker image rm lscr.io/linuxserver/sonarr:latest
```
...
indexers into a service that is gone and reports a health error), and its connection in the subtitle
manager.

The TV library and the downloads are untouched — both are bind mounts and nothing in the uninstall
touches them.
...
| Symptom | Cause / fix |
| --- | --- |
| Container restarts in a loop on first start | It cannot reach the database. `docker logs sonarr` names it: wrong host, missing database, or the client certificate is not readable. Check `docker exec sonarr ls -l /postgres-certs`. |
| `connection requires a valid client certificate` | `./data/certs/sonarr.crt`/`.key` are missing, expired, or the CN does not match the database role. `openssl x509 -in ./data/certs/sonarr.crt -noout -subject -dates`. |
...
| A `.db` file appears under `config/` | The `SONARR__POSTGRES__*` variables were not applied, so it built a local database. Recreate the container with the full environment and delete the file. |
...
| API calls return 401 | Wrong API key. It is whatever `SONARR__AUTH__APIKEY` was set to at start — `docker inspect sonarr --format '{{json .Config.Env}}' \| tr ',' '\n' \| grep APIKEY`. |
...
| "Downloaded episode could not be imported" | The path the download client reports does not exist in this container. Both must mount the same tree at the same path: `/downloads` here must contain the folder the download client says it saved to. |
| Imports copy instead of hardlinking, and the disk fills | The download tree and the library are on different filesystems, or the UIDs differ. Hardlinks only work within one filesystem; check `df <tv-path> <downloads-path>`. |
| Import fails with "permission denied" | `PUID`/`PGID` do not match the owner of the completed folder. `docker exec sonarr id` and `stat -c '%u %g' <downloads-path>/Download/complete/sonarr`. |
...
| Media server never picks up new episodes | The connection's `updateLibrary` is false, or its API key was revoked. Re-test the connection and confirm `docker exec sonarr curl -sf http://jellyfin:8096/health`. |
| Health warning about the download client being unavailable | The download client container is down or was renamed. `docker exec sonarr curl -v http://transmission:9091/transmission/web/`. |
| Everything is slow and the database grows fast | The log database has not been trimmed. `TRUNCATE "Logs";` in the `sonarr-log` database. |
